...
- **Status:** Active. Not yet applied to Services (built before this decision). Build order decided: Step 6 (Solutions) first, built with its direction from the start; Services' schematic-linework retrofit comes after, as its own separate pass — not an oversight if Solutions ships first without Services having been touched yet.
