...
Also active (D-026): Capability 06 (Penetration Testing) prototyped as `CapabilityFindingsMark` (ranked, severity-tagged example findings using the existing `RiskBadge`) at `/services/prototype-v5` — noindex, nothing live touched. A real text-truncation bug (finding labels clipping illegibly next to wider severity badges) was found via screenshot review and fixed (natural wrapping instead of forced single-line truncate) before presenting as final. Awaiting review.
Next: Awaiting Mohammad's review of D-021 (Capability 02, still open) and D-026 (Capability 06). Capabilities 07-08 remain, each needing individually-researched treatment. One "item 5" remains open and unbuilt: regrouping the 8 capabilities into categories (D-015's round, structure/copy only, held pending separate confirmation). Step 8 (How We Work) and any extension of the new visual system to Home/Solutions/Cyber Health all stay on hold until Services is fully approved — expansion is page-by-page, not all at once.
