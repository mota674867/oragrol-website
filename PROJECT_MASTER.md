...
Also active (D-028): Capability 07 (Endpoint Protection/EDR) prototyped as `CapabilityFleetMark` (a device-coverage pictogram — a grid of device icons, mostly "protected," matching the ui-ux-pro-max chart guidance that pictograms beat pie/donut for part-to-whole accessibility) at `/services/prototype-v6` — noindex, nothing live touched. Self-verified against screenshots before reporting; no shared-layout risk found (pictogram isn't taller than the plain orb mark). Awaiting review.
Next: Awaiting Mohammad's review of D-021 (Capability 02, still open) and D-028 (Capability 07). Capability 08 (Incident Response) remains, needing its own individually-researched treatment — the last of the 8. One "item 5" remains open and unbuilt: regrouping the 8 capabilities into categories (D-015's round, structure/copy only, held pending separate confirmation). Step 8 (How We Work) and any extension of the new visual system to Home/Solutions/Cyber Health all stay on hold until Services is fully approved — expansion is page-by-page, not all at once.
