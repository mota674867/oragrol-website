...
Also active (D-024): Capability 05 (Managed Security Services/24-7 MDR) prototyped as `CapabilityMonitorMark` (live-pulse indicator + illustrative alert feed) at `/services/prototype-v4` — noindex, nothing live touched. Note: 05-08 live in `additional-capabilities.tsx`'s shared 4-card grid (not individual full rows like 01-04), so the prototype shows the whole row twice, including a real found-and-flagged consequence (the taller new mark would stretch 06-08's cards via CSS Grid default unless `items-start` is applied on ship — not applied to the live file yet, deliberately). Awaiting review.
Next: Awaiting Mohammad's review of D-021 (Capability 02, still open) and D-024 (Capability 05). Capabilities 06-08 remain, each needing individually-researched treatment. One "item 5" remains open and unbuilt: regrouping the 8 capabilities into categories (D-015's round, structure/copy only, held pending separate confirmation). Step 8 (How We Work) and any extension of the new visual system to Home/Solutions/Cyber Health all stay on hold until Services is fully approved — expansion is page-by-page, not all at once.
