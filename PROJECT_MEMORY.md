...
## Next Steps
0. **Get Mohammad's decision on the Spline hero ring (D-059).** The integration works correctly, but the published scene doesn't yet read as the Oragrol ring/brand and carries an unaddressed Spline watermark — needs either a revised/republished scene or a decision to hold off. `hero.tsx` currently imports `HeroRingSpline` (live); reverting to `PerimeterRing` (D-057, untouched on disk) is a one-line import swap if that's preferred while the scene gets fixed.
