...
## Next Steps
0. **Get Mohammad to find the remaining opaque object/setting in the Spline project itself (D-060).** Confirmed exhaustively that this codebase's CSS and the WebGL context are both already fully transparent — nothing left to fix from this side. The grey box is real scene content on the Spline project.
0. **Get Mohammad's decision on the Spline hero ring (D-059) overall, once D-060's box is resolved.** `hero.tsx` currently imports `HeroRingSpline` (live); reverting to `PerimeterRing` (D-057, untouched on disk) is a one-line import swap if that's preferred while the scene gets fixed.
...
0. **Confirm the Industries dropdown click fix (D-056) live.** Root cause wasn't conclusively pinned — ask Mohammad to click a few Industries items with a real mouse from a non-Industries page and confirm the "closes without navigating" symptom is gone. If it recurs, don't keep chasing it via `claude-in-chrome` (it showed its own click-dispatch unreliability during this investigation) — get his exact browser/OS/repro steps instead.
