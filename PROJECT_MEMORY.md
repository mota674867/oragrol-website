...
**Scope — every language toggle on the site, not just the main header:** searching for the `{locale.toUpperCase()} / {otherLocale.toUpperCase()}` pattern (and its Tailwind `site-header.tsx`/`site-footer.tsx` " | " sibling) turned up **18 files**, because the site has two parallel header systems — the shared Tailwind `SiteHeader`/`SiteFooter` chrome (used by How We Work, Careers/Talent/Partnerships via `oragrol-opportunity-page.tsx`) and each GPT-redesigned page's own hand-rolled header (`industries-client.tsx`, `company-client.tsx`, `faq-client.tsx`, `or-one-client.tsx`, `services-client.tsx`, `ba-client.tsx`, `resources-client.tsx`, `contact-client.tsx`, `home-client.tsx`, plus the 3 legal pages and `resources/[slug]`) — each with its own copy of the toggle. All 18 fixed the same session; fixing only the shared header would have left most of the site's pages still showing the old unhighlighted toggle.

**Fix, same pattern everywhere:** the display convention was already "current locale first, target locale second" (`{locale} / {otherLocale}`), which meant the fix didn't need any conditional branching — just split the one clickable element into two: a static, non-clickable span for the current locale (`locale.toUpperCase()`, colored orange, `cursor:default`) and a real `Link` for the other locale (unchanged target/behavior, inherits the existing hover-orange styling). New CSS: `.language-current{color:var(--orange);cursor:default}` in `app/gpt-pages.css` (covers all 12 GPT-page-style files, which already all `import "@/app/gpt-pages.css"` and already defined `--orange:#ef4d00` — one shared rule, not 12 copies), `.home-language-current` (+ a `.home-language:hover` rule that didn't exist before) added to `homepage-v3.css` for the homepage's separate stylesheet, `.op-lang-current{color:#ef4d00}` added to `oragrol-opportunity-page.tsx`'s own embedded style block, and Tailwind `text-accent`/`font-semibold` classes used directly in `site-header.tsx`/`site-footer.tsx` (no CSS file needed there — `--accent` already resolves to Burnt Orange per `tokens.css`). Existing hover-on-the-whole-toggle CSS rules (`.language:hover{color:var(--orange)}`) still work unchanged after the split, since CSS `:hover` state bubbles from a hovered child up through its ancestors, so hovering the still-clickable other-locale link continues to trigger the wrapper's existing hover rule.

**Files changed:** `app/gpt-pages.css`, `app/[locale]/homepage-v3.css`, `app/[locale]/home-client.tsx`, `app/[locale]/industries/industries-client.tsx`, `app/[locale]/company/company-client.tsx`, `app/[locale]/faq/faq-client.tsx`, `app/[locale]/or-one/or-one-client.tsx`, `app/[locale]/services/services-client.tsx`, `app/[locale]/business-automation/ba-client.tsx`, `app/[locale]/resources/resources-client.tsx`, `app/[locale]/contact/contact-client.tsx`, `app/[locale]/privacy-policy/page.tsx`, `app/[locale]/terms-of-use/page.tsx`, `app/[locale]/accessibility/page.tsx`, `app/[locale]/resources/[slug]/page.tsx`, `app/components/site/site-header.tsx`, `app/components/site/site-footer.tsx`, `app/components/site/oragrol-opportunity-page.tsx`.

**Verification:** `tsc --noEmit` and `npm run lint` both clean (same pre-existing unrelated warnings as every prior phase). A dedicated Playwright check across one route per header system (How We Work/Careers via shared chrome, Homepage, and 4 GPT-page-style routes — Industries/Services/Privacy Policy/Contact), both locales (14 checks): confirmed the active-locale element's computed color resolves to the correct brand orange (`#ef4d00`/`#db5227` depending on which token that page uses) and shows the correct letters (EN on English pages, FR on French pages) — 0 failures. A second check clicked the still-live other-locale link on 4 of those routes and confirmed it lands on the expected opposite-locale URL — 0 failures (the switch itself still works after the markup split). Re-ran the full-site sweep (all 103 routes × 2 locales, 203 checks) as a regression check against the merge — 0 new failures, same 2 known non-issues as the pre-merge sweep (a Playwright response-timing artifact on `/fr`, and `/solutions`'s pre-existing intentional redirect to `/services`).
...
**How the translation was produced:** rather than translating all 67 services serially in one pass, split the 15 categories into 5 balanced batches (~13-15 services each) and delegated each to a parallel subagent with explicit instructions: preserve `code`/`price`/`tier` byte-identical, preserve `**bold**`/`\n\n` markdown structure in `problem`, translate `deliverables`/`benefits` arrays item-for-item with exact order/count, natural confident B2B security/automation marketing register (not stiff literal French, not legal-register), and the same terminology conventions used everywhere else on this site (brand names ORAGROL/OR ONE/Cyber Health Score/My Scope stay English; recognized security/tech/certification jargon — SOC 2, PCI-DSS, MFA, API, SaaS, Zero Trust, "workflow", etc. — stays English per the site's established bias). Two agents independently discovered and reused the project's existing `ORAGROL_Services_FR_Translation.md` / `ORAGROL_ALL_64_SERVICES_MASTER_COMPILATION.md` docs for already-established "Simple Name" translations (Device Guard → Protection des appareils, etc.), keeping this new content consistent with names already shown on the `/services` and `/business-automation` hub pages. All 5 batch outputs were programmatically validated before merging (JSON structure match, `code`/`price`/`unit` untouched, every translatable field actually differs from English, array lengths match) — one false-positive flag (`Cyber Health Check` left identical, correctly — it's a branded assessment name) and zero real issues found across all 67 services.

**Architecture:** `oragrol-services-data.fr.json` (new, `app/data/`) is a full French counterpart of `oragrol-services-data.json`'s exact structure — same reasoning as the legal pages: this is bulk structured content (67 services × ~8 fields, with markdown), not a set of independent UI strings, so a parallel data file beats flattening into `messages/{en,fr}.json`. `services-data.ts`'s accessors (`getServicesTier1`, `getBusinessAutomationTier2`, `getAllCategories`, `getServiceBySlug`) all gained a `locale: "en" | "fr" = "en"` param defaulting to `"en"` — every existing call site (the already-bilingual `/services`/`/business-automation` hub pages, which source their own display copy from next-intl's `Services`/`BusinessAutomation` message namespaces, not this JSON) needed zero changes. Only the two `[code]/page.tsx` routes and `ServiceDetail` actually pass `"fr"` through. `ServiceDetail` (`app/components/sections/services/service-detail.tsx`) gained an `isFr` prop for its own hardcoded field labels ("The Challenge"/"What Oragrol Does"/"What You Get"/"Benefits"/"The Outcome"/"Next step"/"Talk to Oragrol" → French) and switched its breadcrumb `Link` from plain `next/link` to `@/i18n/navigation`'s locale-aware `Link`; the "Services"/"Business Automation" breadcrumb root labels stay English in both locales, matching the site's established nav-label convention (site-header `NAV_LINKS`, `OragrolMegaNav`'s `NAV_ITEMS`). Both `[code]/page.tsx` files converted `generateMetadata` to be locale-aware with real `languageAlternates`.

**Pricing decision:** `price` and `tier` are byte-identical between the EN/FR JSON files — the actual dollar figure must never diverge between locales (a real pricing-integrity risk for an MSSP, not just a translation nicety). `unit` is the one field partially translated: its currency suffix stays untouched (`/mo`, `/user`, `/API`, `/app`, `/model`, `/priv-user`, `/workflow`, `/yr` → `/mois`/`/an` translated, the noun suffixes left as compact technical shorthand per the site's jargon convention), but the ~6 full-English billing-type words (`one-time` → `ponctuel`, `project` → `projet`, `per engagement` → `par mandat`, `per annual engagement` → `par mandat annuel`, `starting fees` → `frais de départ`, `per incident; optional retainer from $500/month` → the French equivalent) are translated — those carry no pricing-divergence risk since they're not numbers, and leaving "$4,500 — one-time" on a French page would have read as an unfinished translation.

**Verification:** `tsc --noEmit` clean, `npm run lint` clean (same pre-existing unrelated warnings as every other phase). Playwright: a 15-route representative sample (one service per category, both `/services/[code]` and `/business-automation/[code]`) × 2 locales — 0 console/page errors, correct field labels, correct breadcrumb. Then a full sweep of **all 67 service routes** in French (and, as a regression check, all 67 again in English) — 0 failures, 0 `MISSING_MESSAGE`, 0 console errors across all 134 checks. Two full-page screenshots reviewed visually (`/fr/services/c05-s01`, `/fr/business-automation/c12-s03`) — confirmed correct French rendering including natural-reading pricing ("$8/utilisateur/mois", "$7,000 — projet"); a visual gap below the bullet lists in the screenshots was confirmed via `body.innerText()` to be the known scroll-triggered `Reveal`-animation Playwright quirk (documented in Phase 2j/2k), not missing content — the Outcome section and CTA text are present and correctly translated in the DOM. Hub pages (`/services`, `/business-automation`, both locales) spot-checked for regressions — none found.

**Files changed:** `app/data/oragrol-services-data.fr.json` (new), `app/components/sections/services/services-data.ts`, `app/components/sections/services/service-detail.tsx`, `app/[locale]/services/[code]/page.tsx`, `app/[locale]/business-automation/[code]/page.tsx`.

**Not done / flagged, not fixed:** like the legal pages, this is AI-translated business copy (not legal text this time, but still real client-facing pricing/service copy) and would benefit from a native-French business review before being treated as final, particularly the coined French names for services that had no prior precedent in the existing translation docs (flagged per-batch by the translating agents in this session's tool-call history, e.g. "Admin Shield" → "Bouclier des comptes privilégiés", "Trust Guard" → "Protection Zero Trust").

**This was the last major content-translation phase.** With Phase 2l (legal pages + footer) and Phase 2m (67 services) both shipped, essentially the entire site's page content has now been translated across this project's Phase 2a-2m sequence. What's left: Task #27 (a final consolidated cross-site Playwright verification pass, not yet run as one comprehensive sweep — though every individual phase has already been verified on its own), and confirming all outstanding patches (13 through this session's newest) have actually been applied on Mohammad's machine and pushed, since origin was still 3-4 commits behind as of this session.

---

## Session — Full-site bilingual review, Phase 2l: Legal pages (Privacy Policy, Terms of Use, Accessibility) fully bilingual + site-wide SiteFooter i18n fix — 2026-09-22

**Completed:** Translated all 3 legal pages (`/privacy-policy`, `/terms-of-use`, `/accessibility`) from `ORAGROL_Legal_Pages_FR_Translation.md`. **Doc's own explicit caveat, surfaced here rather than silently resolved:** these are AI-translated legal documents and need a real legal/translation review before being treated as the binding French version — especially given Quebec's Bill 96 requirements. Terminology followed the doc: PIPEDA → LPRPDE, CASL → LCAP, AODA kept as AODA with the French title alongside (+ "LAPHO"), WCAG kept as WCAG, "Office of the Privacy Commissioner of Canada" → "Commissariat à la protection de la vie privée du Canada (CPVP)".

**Architecture decision (deliberate departure from the messages/{en,fr}.json pattern used everywhere else):** these 3 pages were 100% static server components before this phase — hardcoded English content objects (`PRIVACY_POLICY`, `TERMS_OF_USE`, `ACCESSIBILITY_STATEMENT`) with a `paragraph`/`list`/`table` block structure and inline markdown (`**bold**`, `*italic*`, `[text](url)`), rendered through a shared `renderInline`/`*Blocks` pair per page. Rather than flattening ~30 sections of structured legal content into flat i18n message keys, each content file now exports `_EN`/`_FR` versions of the same content object plus a `get*(locale)` selector (e.g. `getPrivacyPolicy(locale)` in `ORAGROL_PrivacyPolicyContent.ts`). Rationale: keeps each language's full legal text as one reviewable object (easier to hand to a lawyer later), and avoids forcing table/list block structures through `t.raw()` JSON in messages files. All 3 `page.tsx` files converted from static `export const metadata` + sync default export to `generateMetadata({params})` + async `export default async function Page({params})`, staying Server Components (no page.tsx + `*-client.tsx` split needed — next-intl's `Link` from `@/i18n/navigation` works fine server-side, and these pages need no client interactivity beyond the `<details>` mobile TOC, which needs no JS).

**Fixes made along the way:**
- Header wordmark + breadcrumb "Home" link: were plain `next/link`, not locale-aware — switched to `@/i18n/navigation`'s `Link`.
- Language toggle: was a dead `<button className="language">EN / FR</button>` (`aria-hidden`, did nothing) on all 3 pages — now a real `Link` with `locale={otherLocale}`, same pattern as Company/Industries/Resources/Contact.
- `renderInline` (the markdown-lite parser shared by all 3 pages) gained an `isFr` param: internal body links (the `[Contact page](/contact)`, `[Privacy Policy](/privacy-policy)` cross-references embedded in the legal text) now get a manual `/fr` prefix on the French pages — these render through arbitrary parsed text, not through `Link` itself, so this couldn't be fixed by an import swap alone. Verified via Playwright: an internal link on `/fr/terms-of-use` now resolves to `/fr/privacy-policy`, not `/privacy-policy`.
- All UI chrome translated: breadcrumb labels, "ON THIS PAGE"/"IN PLAIN LANGUAGE" headings, "EFFECTIVE"/"LAST UPDATED" date labels, search button aria-label, TOC nav aria-label, Accessibility page's dark contact-callout card (heading/description/location/button label, `href` also `/fr`-prefixed).

**Real bug found and fixed outside the original phase scope — `app/components/site/footer.tsx` (`SiteFooter`, used by every single GPT-redesigned page — Home, Services, Business Automation, OR ONE, Industries, Resources, Company, Contact, Cyber Health, FAQ, all 3 legal pages, Careers/Talent/Partnerships):** despite already importing the locale-aware `Link`, this component had **zero actual text translation** — every string (SITEMAP/COMPANY/LEGAL/NEWSLETTER headings, the newsletter intro/placeholders/button/consent/success/error copy, social aria-labels, the PROTECT/AUTOMATE/UNIFY tagline) was hardcoded English on every page's French version site-wide. Confirmed via a live `/fr/company` screenshot showing an all-English footer before the fix. This was caught by chance while screenshotting the new legal pages, not something the Legal Pages translation doc covered — it's shared infrastructure, not page-specific content. Fixed: added `useLocale` + `isFr` ternaries for all sentence-level UI copy. Left in English (matching the established nav-link-label convention used everywhere else — site-header, the old site-footer.tsx, NAV_ITEMS): the sitemap/company/legal link **labels** themselves (Home, Services, Company, Privacy Policy, ...) — these are page names, not sentence copy, same treatment as every other nav array on the site. Verified via Playwright across `/fr/company`, `/fr/services`, `/fr/privacy-policy`, `/fr`, `/fr/how-we-work`, `/fr/careers`, `/fr/resources`, and the English equivalents — 0 console/page errors, footer now renders "PLAN DU SITE / ENTREPRISE / MENTIONS LÉGALES / INFOLETTRE" on every French page. (The homepage uses a visually distinct footer variant with its own already-translated copy — confirmed unaffected/untouched.)

**Files changed:** `app/[locale]/privacy-policy/page.tsx`, `app/[locale]/privacy-policy/ORAGROL_PrivacyPolicyContent.ts`, `app/[locale]/terms-of-use/page.tsx`, `app/[locale]/terms-of-use/ORAGROL_TermsOfUseContent.ts`, `app/[locale]/accessibility/page.tsx`, `app/[locale]/accessibility/ORAGROL_AccessibilityContent.ts`, `app/components/site/footer.tsx`.

**Verification:** `tsc --noEmit` clean, `npm run lint` clean (only pre-existing unrelated warnings). Playwright against the dev server: all 3 legal pages × 2 locales — 0 console errors, 0 page errors, no `MISSING_MESSAGE`/`IntlError` strings, language-toggle link present and correctly locale-targeted (routes through the pre-existing `/en/...` → `/...` middleware redirect for `localePrefix: "as-needed"`, same behavior already present on every other shipped bilingual page, e.g. `/fr/company`'s own language link — not a regression), internal body links correctly `/fr`-prefixed. Full-page screenshots taken and visually reviewed for all 6 legal-page variants — French renders correctly (headings, TOC, tables, contact callout). Footer fix spot-checked across 7 additional already-shipped pages (EN + FR) with 0 errors and no `MISSING_MESSAGE`.

**Not done / flagged, not fixed:** the translation itself has not had a human legal/translation review (see caveat above — this is the doc's own explicit flag, repeated here per the project's honesty rule). No other issues found.

**Next up:** Task #26 (final sanity pass on `/solutions`, `/how-we-work`, `/canada-coverage-study`, `/style-guide` — already largely de-risked by earlier spot-checks), Task #27 (final cross-site Playwright verification pass), and Task #29 (Phase 2m — the 67 service detail pages, `services/[code]` + `business-automation/[code]`, still untranslated body content — flagged as the largest remaining item, no existing translation doc).

---

## Session — Full-site bilingual review, Phase 2k: Careers/Talent/Partnerships (shared OragrolOpportunityPage, all tabs and forms) fully bilingual — 2026-09-22

**Completed:** Translated the 3 standalone Opportunity pages (`/careers`, `/talent`, `/partnerships`) from `ORAGROL_Careers_Talent_Partnerships_FR_Translation.md` — all 3 pages share one component, `app/components/site/oragrol-opportunity-page.tsx` (`OragrolOpportunityPage` + `ApplicationForm`), which had zero locale-awareness before this phase (no `useTranslations`, plain `next/link`, no language switcher at all — these pages carry no shared site nav/footer by design).

**Key technical constraint, same pattern as the Cyber Health profile selects (Phase 2h):** the `content[page].categories` arrays (`"Sales & Growth"`, `"Certified Specialists"`, etc.) are canonical, ENGLISH, and used as real state (`useState<string>`, radio `value`, every `category === "..."` conditional throughout the render logic, and the submitted `ApplicationPayload.category`) — these must never be translated or the submission payload silently changes. Built a `categoryLabel()` helper (`t.raw(`pages.${page}.categories`)`, index-paired against the canonical array, same index-pairing pattern used throughout this project) so only the *displayed* label is French — the underlying value stays stable. Same fix applied to the `specialties` array's 4 `<option>`s in the Specialty select (added explicit `value={title}` with translated child text — previously had no explicit `value`, so it defaulted to the option's own text content, meaning a translated label would have silently changed what got submitted).

**New `Opportunity` namespace in `messages/en.json`/`fr.json`** covers every shared form-field label (name/email/country/city/phone/specialty/applying-as/etc.), all per-page/per-category context copy (careers' 3 tabs incl. the 4 specialist credentials, talent's 2 tabs, partnerships' 4 tabs), description-label logic, word-counter text, file-validation error strings, consent/privacy copy, the partnerships disclaimer, and all 3 submit-button/status-message variants — structured to mirror the component's own conditional branching exactly (`formTitle.{career,idea,talent,other}`, `descriptionLabel.{experience,idea,expertise,opportunity}`, etc.) rather than one flat page-level blob.

**Two more latent-link bugs found and fixed, same class as Resources' Phase 2i catch:** all 3 `*-client.tsx` files' wordmark logo used plain `next/link`'s `Link` (would drop a French visitor back onto the English homepage on click) — switched to `@/i18n/navigation`'s `Link`. Same fix for the component's own `privacyUrl`/`termsUrl` links (both the `.op-legal` footer and the inline privacy-note link) — were plain `<a href={privacyUrl}>` tags, now locale-aware `Link`s.

**Added a language switcher these 3 pages never had** (small `Link`+`locale` toggle appended to the existing `.op-legal` row, next to Privacy/Terms — no new chrome invented, matches the pages' deliberately minimal, logo-only design).

**Verification performed:** `tsc --noEmit` clean, `npm run lint` clean (0 errors, same 7 pre-existing warnings), JSON valid. Playwright against the dev server: all 6 route×locale combinations (careers/talent/partnerships × en/fr) 200, zero console/page errors, no `MISSING_MESSAGE`, every category tab on every page clicked through and screenshotted. Visually confirmed full-form French rendering for careers' Certified Specialists tab (specialty select + all 4 credentials + form fields + consent + submit button), talent's Propose an Idea tab, and partnerships' Regional or Branch tab — all fields, labels, counters, file notes, privacy/consent copy and the partnerships disclaimer correctly translated, with the language switcher present in the footer. Confirmed EN routes unchanged (`/careers` still shows "Build what comes next." and "Sales & Growth" verbatim).

**Files changed:** `app/components/site/oragrol-opportunity-page.tsx`, `app/[locale]/{careers,talent,partnerships}/page.tsx` (static `metadata` → `generateMetadata`), `app/[locale]/{careers,talent,partnerships}/{careers,talent,partnerships}-client.tsx` (Link import fix only), `messages/en.json`/`fr.json` (new `Opportunity` namespace), `PROJECT_MEMORY.md`.

**Still open:** Legal pages (Phase 2l, translation doc ready, flagged for a real Bill 96/legal review before publishing), the 67-service detail-page catalog (Phase 2m, no existing translation doc, largest remaining item), then the final live Playwright pass across every bilingual page/form before reporting back to Mohammad. Not yet delivered to Mohammad as a patch — immediate next step.

---

## Session — Full-site bilingual review, Phase 2j: How We Work wired + old SiteHeader/SiteFooter/EmergencyCta chrome given a real language switcher — 2026-09-22

**Context:** After the Resources patch (Phase 2i, `9ce0b7d`) was confirmed applied/pushed, Mohammad said "you do it" — delegating a full bilingual review of the whole site rather than reviewing page-by-page himself. This starts that review. Cross-referenced the git log's phase commits against every route under `app/[locale]/` (via `find` + a second pass including the 3 dynamic routes) to find which pages have NO bilingual-phase commit: `accessibility`, `canada-coverage-study`, `careers`, `how-we-work`, `partnerships`, `privacy-policy`, `solutions`, `style-guide`, `talent`, `terms-of-use`, plus `services/[code]`'s own bilingual status was unconfirmed.

**Read the project's own `ORAGROL_Pending_Items_Register.md` (2026-09-21, newer than this repo's own PROJECT_MEMORY assumptions) to avoid re-litigating already-made decisions — it resolves several of the above immediately:**
- `/solutions` = alias of the already-translated Services page, `/canada-coverage-study` = dead route (fallback content), `/style-guide` = explicitly marked internal/disposable on the page itself. **None of the three need translation** — confirmed by directly reading their code too (see below).
- `/how-we-work` = a real, live, orphan page (not linked in the footer nav, only found by directly checking known routes) — **needs translation**, and a full translation doc already exists on file: `ORAGROL_HowWeWork_FR_Translation.md`.
- **Correction to this repo's own stale `PROJECT_MEMORY.md` note** ("Legal pages remain deliberately English-only per Mohammad's earlier decision," written in an earlier phase): the newer Pending Items Register supersedes this — Privacy Policy/Terms of Use/Accessibility were added to scope in the 2026-09-21 full-site inventory, a combined translation doc exists (`ORAGROL_Legal_Pages_FR_Translation.md`, flagged as needing a real legal/Bill 96 review before publishing), and Careers/Talent/Partnerships also got a translation doc (`ORAGROL_Careers_Talent_Partnerships_FR_Translation.md`). These are real open items, not settled exclusions — queued as the next phases (2k, 2l).

**New gap found that the Pending Items Register's own full-site audit missed:** `services/[code]` and `business-automation/[code]` (the individual service detail pages — all 67 services across both routes) share one `ServiceDetail` component and `services-data.ts`, and neither is locale-aware at all — no `locale` param in `generateMetadata`, no `useTranslations`, plain `next/link`. `ORAGROL_Services_FR_Translation.md` only covers the `/services` hub page's package/à-la-carte summary table, not the individual services' full detail-page bodies (`problem`/`what_we_do`/`deliverables[]`/`benefits[]`/`process`/`outcome`, sourced from `app/data/oragrol-services-data.json`, 42 tier-1 + 25 Business Automation = 67 services). No existing translation doc covers this depth — flagged as its own phase (2m), the largest remaining item in the whole bilingual project.

**This phase's actual work — How We Work + the legacy chrome it rides on:**
- `app/components/sections/how-we-work/{hero,stage-sequence,closing-cta,cycle-visual}.tsx` — all 4 converted to `"use client"` + `useTranslations("HowWeWork.*")`. `stage-sequence.tsx` and `cycle-visual.tsx` both had module-level `const STAGES = [...]` arrays mixing structural data (`n`/`angle`) with English display text (`label`/`copy`) — split into a structural-only array plus translated items merged in at render time, same index-pairing pattern used throughout this project. **`cycle-visual.tsx` was a real catch**: it's a separate component from `stage-sequence.tsx` with its OWN hardcoded copy of the same 4 stage labels/one-liners (confirmed via a live FR screenshot showing the hero's ring visual still in English after `stage-sequence.tsx` alone was translated) — easy to miss since it doesn't look like body content, it's the hero's SVG diagram.
- `app/[locale]/how-we-work/page.tsx` — static `metadata` → `generateMetadata({params})`, same pattern as every prior page.
- `messages/en.json`/`fr.json` — new `HowWeWork` namespace (`hero`, `stages.stageLabel`+`items[4]`, `cycle.items[4]`, `closing`).
- **This page intentionally keeps the OLD `SiteHeader`/`SiteFooter`/`EmergencyCta` chrome** (it was never part of the GPT V38 redesign — `site-chrome.tsx`'s `REDESIGNED_ROUTES` set doesn't include it), rather than rebuilding it with the new `OragrolMegaNav` pattern every other bilingual page uses. Reasoning: How We Work's body still uses the OLD Tailwind-token design system (`Section environment=`, `Container`, `Reveal`, `H1`/`H3`/`Text`) — wrapping it in the NEW GPT-style header (`.industry-header`/`gpt-pages.css`) would visually clash and amounts to a redesign, not a translation task. Kept in scope for THIS page's own body content only.
- **Old chrome fix (affects `how-we-work`, `solutions`, `canada-coverage-study`, `style-guide` — the only 4 routes still on this chrome):** `site-header.tsx`'s two-tier header had a literally `aria-hidden="true"`, non-interactive `<span>EN | FR</span>` — its own comment even said "still the same static/decorative label since full locale routing isn't built yet." `site-footer.tsx` (a DIFFERENT, older component from the `footer.tsx` every redesigned page uses) had the identical dead span. Both replaced with a real `Link`/`locale`-switch, same mechanism as every converted page's header. Also translated `site-header.tsx`'s 2 CTA button strings ("Get Cyber Score"/"Get Your Cyber Health Score" → "Obtenez votre Cyber Health Score" for both, reusing the FR wording already locked via `SharedCta.getCyberHealthScore` elsewhere), `site-footer.tsx`'s tagline and copyright line, and `emergency-cta.tsx`'s single "Under attack? Contact us now" string (all 3 files needed `"use client"` added — `site-footer.tsx` and `emergency-cta.tsx` didn't have it before, but were already only ever rendered from `site-chrome.tsx`, itself `"use client"`, so this makes explicit what was already implicitly true, not a new client boundary). **Nav-link labels in both old-chrome components deliberately stay English** (Services/Business Automation/Solutions/Industries/Resources/Company in the header; Company/Resources/Legal columns in the footer), matching the site-wide `NAV_ITEMS` convention already established and accepted everywhere else. **Did NOT fix** the footer's `LEGAL_LINKS` — they point at `/legal/privacy`, `/legal/terms`, `/legal/cookies`, `/legal/accessibility`, none of which exist as real routes (the real pages are `/privacy-policy`, `/terms-of-use`, `/accessibility`, and there is no cookie-policy page anywhere on the site) — a genuine pre-existing English-side bug, but out of scope for a bilingual-translation pass and not something I should silently redesign; flagging here for Mohammad rather than fixing.

**Verification performed:** `tsc --noEmit` clean, `npm run lint` clean (0 errors, same 7 pre-existing warnings). `npm run build` still fails in this sandbox purely on Google Fonts network fetch (pre-existing sandbox limitation, unrelated to any code here — confirmed dev server + Playwright is this project's actual verification method per `CLAUDE.md` §7). Playwright against the dev server: `/how-we-work` and `/fr/how-we-work` both 200, zero console/page errors, no `MISSING_MESSAGE`; full-page and scrolled screenshots on `/fr/how-we-work` confirm the hero, the ring visual, all 4 stage-sequence entries, and the closing CTA all render in French (scroll-triggered `Reveal` animations don't fire in a single `fullPage` screenshot — confirmed via `body.innerText()` that the French text was present in the DOM regardless, then scrolled screenshots showed it rendered correctly). Also spot-checked `/fr/solutions`, `/fr/canada-coverage-study`, `/fr/style-guide` (all still on the same old chrome) — all 200, zero errors, confirming the header/footer/emergency-CTA changes didn't regress the 3 out-of-scope routes. English routes unchanged (English CTA/tagline/copyright/EN|FR-visible-text strings are the literal originals inside the `isFr` ternaries' `false` branch).

**Still open, not done this phase:**
- Task list for the rest of the full-site review: Careers/Talent/Partnerships (Phase 2k, translation doc ready), Legal pages (Phase 2l, translation doc ready but flagged for a real Bill 96/legal review before publishing), the 67-service detail-page catalog (Phase 2m, no existing translation doc, largest remaining item), then a final live Playwright pass across every bilingual page/form (Contact, Cyber Health quiz, the 3 Opportunity forms) before reporting back to Mohammad.
- `services/[code]` dynamic-route bilingual status: confirmed NOT done (see above) — this is new information, not previously tracked anywhere.
- Not yet delivered to Mohammad as a patch — immediate next step for this phase specifically; larger phases (2k/2l/2m) still to come before the full review is reportable as complete.

---

## Session — Resources hub + all 16 articles fully bilingual (Task #21, Phase 2i) — 2026-09-22

**Completed:** Confirmed Company (Phase 2g, commit `5f2983f`) and Cyber Health Assessment (Phase 2h, commit `ee7d671`) patches applied/pushed by the user. Then built the largest and final item in Task #21's EN/FR scope: the Resources hub page and all 16 full article detail pages, fully bilingual.

**Architecture found (not previously documented):** Resources has three separate English content sources, only two of which are live:
- `app/components/sections/resources/articles-data.ts` (8 articles, structured-block format) + `app/components/sections/resources/resources-explorer.tsx` — **dead code**. Confirmed via repo-wide grep that nothing imports `resources-explorer.tsx` or references `ResourcesExplorer` outside its own file. Left entirely untouched, same precedent as `industry-details.ts` in an earlier phase.
- `app/[locale]/resources/[slug]/ORAGROL_ResourceArticles.ts` (16 articles, markdown-string `body`) — the live source for `/resources/[slug]/page.tsx`. This was a **plain, fully English, non-locale-aware Server Component** before this session (no `params.locale`, `Link` from `next/link`, hand-rolled flat `nav` array, dead `<button>EN / FR</button>`) — the one dynamic detail-page architecture in the project not yet touched by any prior bilingual phase.
- `app/[locale]/resources/resources-client.tsx` — the hub page's own independent, hand-maintained 16-entry `resources: Resource[]` array (title/type/topic/industry/read/summary/href), unrelated to either of the above two files. This is the sole source for the hub's two card grids.

**Files changed:**
- `app/[locale]/resources/[slug]/ORAGROL_ResourceArticles.ts` — added French counterpart fields to every one of the 16 article objects, inline in the same object rather than a parallel array/file: `titleFr`, `seoTitleFr`, `seoDescriptionFr`, `typeFr`, `topicFr`, `industryFr`, `summaryFr`, `bodyFr`, `nextActionFr`, `primaryCtaFr`, `sourcesFr`. Pure additions — verified via `git diff --stat` that this file has 251 insertions and **0 deletions**; every EN field (`title`, `body`, `slug`, `sources`, etc.) is untouched byte-for-byte. `bodyFr` preserves the exact `## `-heading / `- `-bullet / `1. `-numbered-list markdown structure the shared `parseBody()` renderer depends on (including the special-cased 4-item risk-tier grid on article 1, reformatted as `**0–39: Critique**` etc. with no space before the colon so it still matches the renderer's regex). `estimatedRead` has no Fr counterpart (e.g. "3 minutes" reads identically in French); `slug`, `number`, and every `href` stay canonical/English-only since they're machine-facing (routes, `generateStaticParams`, CTA destinations, `getRelatedArticles` scoring).
- `app/[locale]/resources/[slug]/page.tsx` — converted to locale-aware: `PageProps` now carries `locale`; `generateMetadata` picks `seoTitleFr`/`seoDescriptionFr` and adds `languageAlternates`/OG locale/alternateLocale, same pattern as every other page; the component builds one `T` object of localized fields (title/type/topic/industry/summary/body/nextAction/primaryCta/sources) selected by `isFr`, and `parseBody(T.body)` instead of `parseBody(article.body)`. `Link` switched from `next/link` to `@/i18n/navigation`'s `Link` — this also **fixed a latent bug**: the primary-CTA button and every related-article card previously used plain `<a href="/...">` tags, which would have dropped a French visitor back onto the English page on click (no `/fr` prefix). Nav labels (Services/Business Automation/OR ONE/Industries/Resources/Company) deliberately stay English on both locales, matching the site-wide `NAV_ITEMS` convention already established and accepted on every other page. Added a working `Link`-based language switcher (was a dead `<button>EN / FR</button>`).
- `app/[locale]/resources/page.tsx` — converted from static `metadata` export to `generateMetadata({params})`, same fix as every prior page.
- `app/[locale]/resources/resources-client.tsx` — rewired to `useTranslations("Resources")` + `useLocale()`, `Link`/`usePathname` from `@/i18n/navigation` (was `next/link`/`next/navigation`). The canonical `resources` array (hrefs, order, the `.slice(0,6)`/`.slice(6)` grid split) is untouched; display text (`title`/`type`/`summary`) comes from `t.raw("items")`, index-paired 1:1 against `resources` in the same order — same pattern used for every index-paired array in this project. Fixed the dead `<button>EN / FR</button>` into a working switcher.
- `messages/en.json` / `messages/fr.json` — new `Resources` namespace (`header`, `hero`, `essential`, `newGuides`, `readAction`, `readSuffix`, `items[16]`), sourced from `ORAGROL_Resources_FR_Translation.md`. Also fixed `SharedCta.resources` in `fr.json`, which was still the English placeholder (eyebrow/headline/secondaryLabel) — the **last remaining `SharedCta.*` gap** of this kind; every other `SharedCta` entry (`or-one`, `industries`, `contact`, `faq`, `company`) was already fixed in earlier phases. Verified EN/FR structural parity for the new namespace with a recursive Python diff script (no mismatches).

**Naming/terminology decisions applied faithfully from the translation doc (not re-litigated):**
- **PIPEDA/LPRPDE**, the doc's own flagged "biggest judgment call": body text uses **LPRPDE** with "(PIPEDA)" on first mention per article (heaviest in article 6, "Understanding PIPEDA", and referenced in articles 5, 9, 13). Source-citation **document titles** stay in English per the doc's explicit note that matching French OPC page titles/URLs were never verified — flagged for Mohammad, not resolved by guessing.
- **Source citation org names ARE translated** (Centre canadien pour la cybersécurité, Commissariat à la protection de la vie privée du Canada, Bureau d'assurance du Canada, Statistique Canada, Innovation, Sciences et Développement économique Canada, GRC), matching the doc's global rule #5/#6 — applied uniformly even in the ~3 articles (5, 6, 13) where the doc's own per-article write-up inconsistently left the org name in English; the doc's stated global rule was treated as authoritative over those few inconsistent instances.
- Technical acronyms/protocols (MFA, SPF, DKIM, DMARC, HTTPS, VPN, CRM, PCI-DSS, literal `p=none`/`p=quarantine`/`p=reject`) and Microsoft/Google product feature names (Security Defaults, Conditional Access, Secure Score, 2-Step Verification) kept in English, per the doc.
- Brand marks (Cyber Health Score, OR ONE, ORAGROL, Business Automation) kept English throughout, per the doc and project-wide convention.
- Article type-tag translations (Checklist→Liste de vérification, Executive Playbook→Guide d'intervention pour dirigeants, Policy Guide→Guide de politique, Technical Checklist→Liste de vérification technique, Industry Brief→Note sectorielle, Executive Brief→Note de synthèse pour dirigeants, Assessment Guide→Guide d'évaluation, Executive Guide→Guide pour dirigeants) sourced from each article's own "Tags:" line in the doc, since the doc's hub teaser table left the Type-tag column untranslated (an oversight in the doc, not a real per-article decision).

**Verification performed:** `tsc --noEmit` clean (0 errors) after every stage. `npm run lint` clean — 0 errors, same 7 pre-existing warnings as every prior phase. Dev server: all of `/resources`, `/fr/resources`, and both English and French `/resources/<slug>` for 3 different articles (including the risk-tier-grid article, the bulleted-checklist article, and the numbered-list article) returned 200 with no `MISSING_MESSAGE`/`IntlError`. Playwright: screenshotted hub (EN/FR) and 3 article detail pages (EN/FR) at 1440px, zero console/page errors across all 6 routes; visually confirmed FR risk-tier grid (Critique/Élevé/Moyen/Faible), FR numbered/bulleted lists, FR TOC-anchor↔heading-id matching, and the FR language-switcher chip all render correctly. Confirmed EN article/risk-grid text (`Critical`/`High`/`Medium`/`Low`, `<h1>What Is a Cyber Health Score?</h1>`) unchanged on the English routes. `git diff --stat` confirms the data file is pure-addition (251 insertions, 0 deletions) — no EN content was at risk.

**Still open (outside this phase's scope, flagged for the record, not fixed here):**
- PIPEDA source-citation URLs: per the doc's own flag, someone should locate and swap in the real French-language OPC page titles/URLs before this goes fully live — not guessed here.
- Services' own Details-accordion content (`app/lib/services-details-content.tsx`) remains English-only, a separately known-open item from an earlier phase.
- Legal pages remain deliberately English-only per Mohammad's earlier explicit decision.
- PR #13 should stay unmerged into `main` until the whole site is bilingual.

**This closes out the entire expanded EN/FR translation queue** per the translation doc's own final tally: Homepage, Services, Business Automation, OR ONE, Contact+FAQ, Industries, Company, Cyber Health Score quiz, and now Resources (hub + all 16 articles) are all fully bilingual. Task #21 delivers as a 12th patch in this session's series — same build → verify → commit → `git format-patch` → fresh-clone verify → `SendUserFile` → apply/push → resync cycle as every prior page.
