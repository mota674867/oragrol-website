...
const security=["Risk","Weaknesses","Threats","People","Access","Cloud","Data","Apps","AI","Testing"] as const;
function Arrow(){return <span aria-hidden="true">↗</span>}

function HomeClient(){
 const[capability,setCapability]=useState(0),[stage,setStage]=useState(0),[category,setCategory]=useState(0);
 const activeCapability=capabilities[capability],activeStage=stages[stage];
 return <main className="home-v3">
  <header className="home-nav"><Link className="home-wordmark" href="/" aria-label="ORAGROL Global home"><strong>ORAGROL</strong><small>GLOBAL</small></Link><nav aria-label="Primary navigation">{nav.map(([label,href])=><Link href={href} key={label}>{label}</Link>)}</nav><div className="home-actions"><Link className="health-link" href="/cyber-health">Get Cyber Health Score</Link><button className="home-search" aria-label="Search">⌕</button><button className="home-language" aria-label="Change language">EN <span>/ FR</span></button></div></header>
