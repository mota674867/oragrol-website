...
import { Badge, CONTAINER_MAX_WIDTH, H1, Section, Text } from "../../ui";
 ...
 * follow-up to D-072). D-072's version wrapped the whole hero in a large
 * rounded, `bg-surface/40` card with a full-panel `GlowEffect` washed in the
 * accent family at 25% opacity — read as "brown/orange card," the exact
 * thing this pass removes. Composition reference: an external image (dark
 * ground, small logomark + pill eyebrow + large two-tier headline + short
 * description on the left, one large visual bleeding toward the frame's own
 * edge on the right, restrained warm rim-light on the visual only) — used
 * for COMPOSITION only: brand, content, colors, typography, and the visual
 * itself are all ORAGROL's own (current D-068 palette; the brief re-quoted
 * the pre-D-068 retired hex values a third time — flagged again, proceeding
 * on the authoritative tokens.css palette per standing precedent).
 *
...
export function ServicesHero() {
  return (
    <Section environment="dark">
      <div
        className={`mx-auto w-full overflow-x-clip pb-16 pl-6 pr-6 pt-16 sm:pb-20 sm:pt-20 md:pb-24 md:pl-12 md:pr-12 md:pt-24 lg:pr-0 lg:pt-28 ${CONTAINER_MAX_WIDTH["2xl"]}`}
      >
        <div className="grid grid-cols-1 items-center gap-y-16 lg:grid-cols-[1fr_1.3fr] lg:gap-x-10">
          <div>
            ...
            <Reveal delay={0.05}>
              <Badge tone="accent" className="mt-12">
                Services
              </Badge>
            </Reveal>
            <Reveal delay={0.1}>
              <H1 size="xl" className="mt-8 max-w-xl">
