    ...
    <p style={{fontSize:"13px",lineHeight:1.6,margin:"4px 0 0",overflowWrap:"break-word",wordBreak:"break-word",whiteSpace:"pre-wrap",minWidth:0,padding:0}}>
      {parts.map((part,i)=>{
        ...
        return segs.map((seg,j)=>
          seg.startsWith("http")?
          <a key={i+"-"+j} href={seg} target="_blank" rel="noopener noreferrer" style={{color:"inherit",textDecoration:"underline",wordBreak:"break-all"}}>{seg}</a>:
            ...
            <div style={{borderTop:"1px solid #ccc",background:"#f4f1ea",padding:"10px 12px",display:"flex",flexDirection:"column",gap:"6px"}}>
              ...
              <form onSubmit={submit} style={{display:"flex",gap:"8px",alignItems:"flex-end"}}>
                <button type="button" onClick={()=>fileInputRef.current?.click()}
                  style={{border:"1px solid #aaa7a0",background:"#f4f1ea",padding:"8px 10px",cursor:"pointer",fontSize:"14px",flexShrink:0,lineHeight:1}}
                  title="Attach image (JPG, PNG, WEBP — max 5MB)">📎</button>
