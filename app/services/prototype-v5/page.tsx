...
export const metadata: Metadata = {
  title: "Prototype V5 — Capability 06 shell redesign (internal)",
  robots: { index: false, follow: false },
};

/**
 * /services/prototype-v5 — Round 9, Capability 06 (Penetration Testing).
 * See `capability-findings-mark.tsx` for the full reasoning trail (tool
 * findings -> design decision), stated there per instruction.
 *
 * Same whole-4-card-row comparison approach as `/services/prototype-v4`
 * (Capability 05) — the real question for anything in this shared grid is
 * how it sits next to its siblings, not the one card in isolation.
 * Capability 05 is already shipped (D-025) with its own `CapabilityMonitorMark`,
 * so both rows below reflect that as the current baseline; only
 * Capability 06 differs between the two rows. The grid's `items-start`
 * fix (D-025) already covers this new taller mark — no further grid
 * change needed, shown as-is in both rows.
 *
...
function CapabilityGrid({ use06Prototype }: { use06Prototype: boolean }) {
  return (
    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4 lg:items-start">
      {FINALIZING_SERVICES.map((service) => (
        <Card key={service.name} variant="surface" className="flex h-full flex-col gap-3">
          ...
          {service.n === "06" && use06Prototype && <CapabilityFindingsMark />}
          {service.n === "06" && !use06Prototype && (
            <CapabilitySpotlightMark icon={service.icon} className="h-14 w-14" />
          )}
          {service.n !== "05" && service.n !== "06" && (
            <CapabilitySpotlightMark icon={service.icon} className="h-14 w-14" />
          )}
    ...
    <Section environment="light-blue">
      <div className="mx-auto w-full max-w-[1200px] px-6 py-24 md:px-12">
        ...
        <h1 className="mt-4 font-heading text-3xl font-bold text-text-primary md:text-4xl">
          Capability 06 — mark redesign, in context
        </h1>
        <Text size="base" tone="secondary" className="mt-4 max-w-2xl">
          Same 4-card row shown twice — Capability 05 already reflects its shipped treatment (D-025) in both;
          only Capability 06 differs. Nothing in <code>additional-capabilities.tsx</code> is touched by this route.
        </Text>
          ...
          <div className="border-t border-border pt-10">
            <div className="mb-6 flex flex-wrap items-baseline gap-3">
              ...
              <Text size="sm" tone="muted">
                Capability 06 still uses `CapabilitySpotlightMark`, shell disapproved for 04-08.
              </Text>
            </div>
            <CapabilityGrid use06Prototype={false} />
          </div>

          <div className="border-t border-border pt-10">
            <div className="mb-6 flex flex-wrap items-baseline gap-3">
              <span className="inline-flex items-center rounded-full border border-accent/30 bg-accent/10 px-3 py-1 font-body text-xs font-semibold uppercase tracking-wide text-accent">
                Prototype — Capability 06 only
              </span>
              <Text size="sm" tone="muted">
                `CapabilityFindingsMark` (new): ranked, severity-tagged example findings, in place of the orb.
              </Text>
            </div>
            <CapabilityGrid use06Prototype={true} />
          </div>
        </div>
      </div>
    </Section>
