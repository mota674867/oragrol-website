...
export default function ServicesPage({ onAddToScope, isInScope, onDiscussEngagement }: ServicesPageProps) {
  ...
  return <div className="oragrol-services">
    ...
    <section className="os-packages os-dark os-section" id="service-packages" aria-labelledby={`${uid}-packages`}>
      ...
      {SERVICE_PACKAGES.filter(p => p.id !== current.id).map(p => { const info = packagesInfoItems.find(it => it.id === p.id); if (!info) return null; return <span key={p.id} style={{ position: "absolute", top: 0, left: 0, width: "1px", height: "1px", padding: 0, margin: "-1px", overflow: "hidden", clipPath: "inset(50%)", whiteSpace: "nowrap", border: 0 }}>{info.title}: {info.plainText}</span>; })}
    </section>
