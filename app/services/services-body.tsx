...
export default function ServicesPage({ onAddToScope, isInScope, onDiscussEngagement }: ServicesPageProps) {
  ...
  return <div className="oragrol-services">
    ...
    <section className="os-individual os-light os-section" id="individual-services" aria-labelledby={`${uid}-individual`}>
      ...
      <div className="os-service-grid">{INDIVIDUAL_SERVICES.map(s => { const item = serviceSelection(s); const added = isInScope(item); return <article key={s.code} className="os-service-card"><p className="os-eyebrow">{s.billing === "monthly" ? "MONTHLY SERVICE" : "PROJECT SERVICE"}</p><h3>{s.name}</h3><p className="os-description">{s.line}</p><div className="os-card-price">{money(s.price)}<small>{s.billing === "monthly" ? "/mo" : s.billing === "per-application" ? "/app" : ""}</small></div><p className="os-billing">{s.billing === "monthly" ? "Monthly rate · 12-month contract" : s.billing === "per-application" ? "One-time / project · per application" : "One-time / project"}</p><p className="os-standalone">Available standalone.</p><Action disabled={added} perform={() => onAddToScope(item)}>{added ? "Added to My Scope" : "Add to My Scope ↗"}</Action></article>; })}</div>
    </section>
