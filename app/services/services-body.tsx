...
import { packagesInfoItems, individualServicesInfoItems, specialistInfoItems } from "../lib/services-info-content";
  ...
  return <div className="oragrol-services">
    ...
    <section className="os-packages os-dark os-section" id="service-packages" aria-labelledby={`${uid}-packages`}>
      ...
      <div className="os-package-layout"><div className="os-selector" role="group" aria-label="Choose a cybersecurity package">{SERVICE_PACKAGES.map((p, i) => <button key={p.id} type="button" aria-pressed={selected === i} aria-controls={`${uid}-package-panel`} onClick={() => setSelected(i)}><span className="os-selector-top"><span>0{i + 1}</span><span>{p.services.length} SERVICES</span></span><span className="os-package-name">{p.name}</span><span className="os-selector-price">{money(p.price)}<small>/mo</small></span><span className="os-selector-value">{p.value}</span></button>)}</div>
        <div id={`${uid}-package-panel`} className="os-package-panel" aria-live="polite" aria-atomic="true"><p className="os-eyebrow">{current.services.length} INCLUDED SERVICES</p><h3>{current.name}{(() => { const info = packagesInfoItems.find(it => it.id === current.id); return info ? <InfoPopup id={`svc-pkg-${info.id}`} variant="modal" title={info.title} content={info.content} plainText={info.plainText} /> : null; })()}</h3><p className="os-package-value">{current.value}</p><div className="os-package-summary"><div><p className="os-eyebrow">IDEAL FIT</p><p>{current.fit}</p></div><div><p className="os-eyebrow">MONTHLY RATE · 12-MONTH CONTRACT</p><p className="os-main-price">{money(current.price)}<small>/mo</small></p></div></div><p className="os-eyebrow">WHAT&apos;S INCLUDED</p><ul className="os-inclusions">{current.services.map(name => <li key={name}>{name}</li>)}</ul><div className="os-package-bottom"><p>Initial three-month option: {money(current.initialPrice)}/mo. Then moves into a 12-month standard contract.</p><Action disabled={isInScope(selection)} perform={() => onAddToScope(selection)}>{isInScope(selection) ? "Added to My Scope" : "Add to My Scope ↗"}</Action></div></div>
      </div>
      <div className="os-full-reference" style={{ borderTop: "1px solid #3e4245", marginTop: "50px", paddingTop: "35px" }}>
        <p className="os-eyebrow" style={{ marginBottom: "18px" }}>OR5 / FULL DETAILS</p>
        {SERVICE_PACKAGES.map(p => { const info = packagesInfoItems.find(it => it.id === p.id); if (!info) return null; return <div key={p.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 0", borderBottom: "1px solid #3e4245" }}><span style={{ fontSize: "18px" }}>{info.title}</span><InfoPopup id={`svc-pkg-ref-${info.id}`} variant="modal" title={info.title} content={info.content} plainText={info.plainText} /></div>; })}
      </div>
    </section>
    ...
    <section className="os-individual os-light os-section" id="individual-services" aria-labelledby={`${uid}-individual`}>
      ...
      <div className="os-service-grid">{INDIVIDUAL_SERVICES.map(s => { const item = serviceSelection(s); const added = isInScope(item); const info = individualServicesInfoItems[s.code]; return <article key={s.code} className="os-service-card"><p className="os-eyebrow">{s.billing === "monthly" ? "MONTHLY SERVICE" : "PROJECT SERVICE"}</p><h3>{s.name}{info ? <InfoPopup id={`svc-ind-${s.code}`} variant="popover" title={s.name} content={info.content} plainText={info.plainText} /> : null}</h3><p className="os-description">{s.line}</p><div className="os-card-price">{money(s.price)}<small>{s.billing === "monthly" ? "/mo" : s.billing === "per-application" ? "/app" : ""}</small></div><p className="os-billing">{s.billing === "monthly" ? "Monthly rate · 12-month contract" : s.billing === "per-application" ? "One-time / project · per application" : "One-time / project"}</p><p className="os-standalone">Available standalone.</p><Action disabled={added} perform={() => onAddToScope(item)}>{added ? "Added to My Scope" : "Add to My Scope ↗"}</Action></article>; })}</div>
    </section>
    ...
    <section className="os-specialists os-dark os-section" id="specialist-engagements" aria-labelledby={`${uid}-specialists`}>
      ...
      <div className="os-specialist-grid">{SPECIALIST_ENGAGEMENTS.map((s, i) => { const info = specialistInfoItems[s.code]; return <article className="os-specialist-card" key={s.code}><p className="os-eyebrow">0{i + 1} / AVAILABLE BY ENGAGEMENT</p><h3>{s.name}{info ? <InfoPopup id={`svc-spec-${s.code}`} variant="modal" title={info.title} content={info.content} plainText={info.plainText} /> : null}</h3><p className="os-description">{s.line}</p><div className="os-engagement-prices"><p className="os-engagement-price-main">{s.priceLine}</p>{s.secondaryPriceLine ? <p className="os-engagement-price-secondary">{s.secondaryPriceLine}</p> : null}</div><Action perform={() => onDiscussEngagement({ code: s.code, name: s.name })}>Discuss Your Engagement ↗</Action></article>; })}</div>
    </section>
  </div>;
}
