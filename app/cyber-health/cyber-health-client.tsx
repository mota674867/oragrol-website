...
function CyberHealthClient(){
  ...
  if(stage==="intro")return <main className="nch intro">{header("CYBER HEALTH / PRIVATE ASSESSMENT")}<section><span className="nch-or">OR</span><p>5–7 MINUTES · 42 SECURITY QUESTIONS</p><h1>Know where you stand.<br/><i>Know what to do next.</i></h1><p>Build a practical baseline across identity, email, devices, cloud, people, data and governance.</p><button onClick={()=>go("profile")}>Begin assessment ↗</button></section><SiteFooter/></main>;
  ...
  if(stage==="result")return <main className="nch result">{header("YOUR RESULT")}<section><div><p>CYBER HEALTH SCORE</p><strong>{score}</strong><span>/100</span><dl><div><dt>Risk tier</dt><dd>{tier}</dd></div><div><dt>Recommended next step</dt><dd>Private Scope Review</dd></div></dl></div><article><p className="eyebrow">{profile.company||"YOUR BUSINESS"}</p><h1>{score>=80?"Strong foundation.":score>=60?"A workable base with clear gaps.":score>=40?"Material improvement is needed.":"Start with the essentials."}</h1><p>A directional baseline, not a certification or guarantee. Review uncertain and missing controls before deciding what to improve first.</p><button onClick={()=>{setStep(0);go("assessment")}}>Review answers</button>{reportStatus!=="idle"&&<p className="ch-report-status" data-status={reportStatus} role="status">{reportStatus==="sending"?`Preparing your full report for ${profile.email}…`:reportStatus==="sent"?`Your full report has been sent to ${profile.email}.`:reportError}</p>}</article></section><SiteFooter/></main>;
