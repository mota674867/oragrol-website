...
import { orOneDetailsContent } from "../lib/or-one-details-content";

export function RevisedSignature(){return <section id="or-one-signature" className={s.signature} aria-labelledby="or-one-signature-heading"><div className={s.signatureGrid}><div className={s.signatureCopy}><div className={s.signatureLabelRow}><p className={s.signatureLabel}>OR / ONE SIGNATURE</p><span className={s.signatureRule} aria-hidden="true"></span></div><h2 id="or-one-signature-heading">Your vision, precisely engineered.</h2><p>Tell us what your operation needs to achieve. We will design and create a bespoke system around your ambitions—from intelligent workflows and automation to integrations, controls, reporting, and specialised capabilities.</p><p>Each Signature engagement is privately reviewed and shaped into a custom architecture, defined scope, implementation plan, and ongoing service structure.</p><Link href="/contact#enquiry" className={s.signatureCta}>BEGIN YOUR SIGNATURE PROJECT <span aria-hidden="true">↗</span></Link></div><svg className={s.signatureMotif} viewBox="0 0 420 420" aria-hidden="true" focusable="false"><line x1="140" y1="0" x2="140" y2="420" stroke="#45494b" strokeWidth="1"/><line x1="310" y1="0" x2="310" y2="420" stroke="#45494b" strokeWidth="1"/><line x1="0" y1="140" x2="420" y2="140" stroke="#45494b" strokeWidth="1"/><line x1="0" y1="290" x2="420" y2="290" stroke="#45494b" strokeWidth="1"/><path d="M310 290 A170 170 0 0 0 420 155" fill="none" stroke="#5a5e61" strokeWidth="1"/><rect x="140" y="30" width="220" height="330" rx="28" fill="none" stroke="#ef4d00" strokeWidth="2"/><rect x="304" y="284" width="12" height="12" fill="none" stroke="#7a7e81" strokeWidth="1"/></svg></div></section>}
...
['STARTER','Up to 30 Points · one category','$22,000','$999','A focused first system.'],
...
].map(([name,points,build,monthly,line])=>{ const content = orOneDetailsContent[name as string]; return <article key={name}><span className={s.eyebrow}>OR / ONE</span><h3>{name}</h3><p className={s.fit}>{line}</p>{content && <DetailsDialog category="OR ONE" {...content} />}<p className={s.points}>{points}</p><div className={s.fee}><span>BUILD FEE</span><strong>{build}</strong></div><div className={s.monthly}><span>BASE MONTHLY OR SERVICE FEE</span><strong>{monthly}<small>/mo</small></strong></div></article>; })}</div><div className={s.pricingNote}><p>All fees in CAD. Points measure the complexity of your selected capabilities and help determine your system tier. Final scope and monthly fee are confirmed through private review.</p><p>The builder provides a preliminary tier, not a binding quote.</p></div></section>}
